...
def describe_rv_score(rv: float) -> str:
    ...
    if rv < 16:
        return (
            "У вас наблюдается **баланс** между традиционными и "
            "секулярно-рациональными ценностями."
        )
    return (
        "Скорее всего, у вас в приоритете **научные знания**, "
        "**технический прогресс** и уважение к **индивидуальности**."
    )
...
def describe_sv_score(sv: float) -> str:
    ...
    if sv < 10:
        return (
            "Скорее всего, у вас в приоритете **материальная и физическая безопасность**."
        )
    if sv < 14:
...
def format_indices_summary(rv: float, sv: float) -> str:
    ...
    return "\n\n".join(
        [
            (
                f"Ваш индекс **традиционных/секулярно-рациональных ценностей** "
                f"составляет **{rv}**.\n\n{describe_rv_score(rv)}"
            ),
            (
                f"Ваш индекс **ценностей выживания/самовыражения** "
