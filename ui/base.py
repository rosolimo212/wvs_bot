...
from core.questionnaire.factory import build_main_answer_store, build_secondary_answer_store


def build_app_service(config: dict[str, Any]) -> AppService:
    logger = build_logger(config)
    answer_store = build_main_answer_store(config)
    secondary_answer_store = build_secondary_answer_store(config)
    return AppService(
        logger=logger,
        config=config,
        answer_store=answer_store,
        secondary_answer_store=secondary_answer_store,
    )
